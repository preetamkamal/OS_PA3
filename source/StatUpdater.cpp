...
StatUpdater::StatUpdater(DList<PCB> *rq, DList<PCB> *fq, DList<PCB> *bq, Clock *cl, int alg, std::string fn, int tq)
{
    ready_queue = rq;
    finished_queue = fq;
    blocked_queue = bq;
...
void StatUpdater::execute()
{
    ...
    // Simulate I/O completion for blocked processes
    for (int index = 0; index < blocked_queue->size(); ++index)
    {
        PCB *temp = blocked_queue->getindex(index);
        temp->io_burst -= increment;

        if (temp->io_burst <= 0)
        {
            temp->io_burst = 0; // Ensure io_burst doesn't go negative
            PCB temp_pcb = blocked_queue->removeindex(index);
            ready_queue->add_end(temp_pcb);
...
void StatUpdater::print()
{
    ...
    outfile << "| " << std::left << std::setw(colwidth) << "PID" << "| " << std::left << std::setw(colwidth) << "Arrival"
            << "| " << std::left << std::setw(colwidth) << "CPU-Burst" << "| " << std::left << std::setw(colwidth) << "I/O Burst"
            << "| " << std::left << std::setw(colwidth) << "Priority" << "| " << std::left << std::setw(colwidth) << "Finish"
            << "| " << std::left << std::setw(colwidth) << "Waiting" << "| " << std::left << std::setw(colwidth) << "Turnaround"
            << "| " << std::left << std::setw(colwidth) << "Response" << "| " << std::left << std::setw(colwidth) << "Blocked"
        ...
        for (int index = 0; index < finished_queue->size(); ++index)
        {
            if (finished_queue->getindex(index)->pid == id)
            {
                ...
                outfile << "| " << std::left << std::setw(colwidth) << temp.pid << "| " << std::left << std::setw(colwidth)
                        << temp.arrival << "| " << std::left << std::setw(colwidth) << temp.burst << "| " << std::left << std::setw(colwidth)
                        << temp.initial_io_burst << "| " << std::left << std::setw(colwidth) << temp.priority << "| " << std::left << std::setw(colwidth)
                        << temp.finish_time << "| " << std::left << std::setw(colwidth) << temp.wait_time << "| " << std::left << std::setw(colwidth)
                        << turnaround << "| " << std::left << std::setw(colwidth) << temp.resp_time << "| " << std::left << std::setw(colwidth)
                        << (temp.has_blocked ? "Yes" : "No") << "| " << std::left << std::setw(colwidth) << temp.num_context << "|" << std::endl;
                outfile << "---------------------------------------------------------------------------------------------------------------------------------------------------------" << std::endl;
            }
        }
    }
